...
public class ControlRutasServiceImpl implements ControlRutasService {
    ...
    public Respuesta<Page<ControlRutasTablaResponse>> consultarRutas(Pageable pageable) {
        ...
        try {
            ...
            for (ControlRutas rutas : result) {
                ...
                origen.ifPresent(unidadAdscripcion -> rutasTabla.setOrigen(unidadAdscripcion.getNomUnidadAdscripcion()));
        ...
        try {
            ...
            if (result.isPresent()) {
                ...
                if (ruta.getModulo() != null) {
                    ...
                    // todo - recuperar de un servicio de modulo de ambulancia
                ...
                if (ruta.getTripulacion() != null) {
                    ...
                    TripulacionInterfaceResponse tripOp = triRepository.findTripulacionIdVehiculo(ruta.getVehiculo().getIdVehiculo());
                    ...
                    if (tripOp != null) {
                        ...
                        // todo - usar la utileria para crear las respuestas
    ...
    public Respuesta<Integer> crearRuta(ControlRutasRequest rutas) {
        ...
        try {// Pendiente crear el campo foliador....
            // todo - mover a la utileria respponseUtil para que desde ahi se haga la validacion del usuario
            //      - mover la logica al controller
            String user = (String) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            log.info("usuario {}", user);
            if (user.equals("denegado")) {
                response.setError(false);
            ...
            // todo - mover a un servicio para Rutas y que ahi se haga lo de ruta origen y destino
            //Creando la ruta
            ...
            List<String> horas = Utility.getHorarioStringByTurno(rutas.getTurno());
            if (horas.size() == 2) {
                ruta.setTimHorarioInicial(horas.get(0));
                ruta.setTimHorarioFinal(horas.get(1));
            }

            // todo - mover al servicio de solicitudTrasladoService y de ahi recuperar la informacion
            //Obtenemos solicitud
            final Integer idSolicitudTraslado = rutas.getIdSolicitudTraslado();
            SolicitudTraslado solicitud = solRepository
                    .findById(idSolicitudTraslado)
                    .orElseThrow(() ->
                            new EntityNotFoundException("No se econtro la solicitud: " + idSolicitudTraslado));

            ruta.setIdOrigen(solicitud.getCveOrigen());
            ruta.setIdUnidadDestino(solicitud.getCveDestino());
            ruta.setIdUnidadSolcitante(solicitud.getIdUnidadSolicitante());

            log.info("Actualizando estatus a asignada ");
            // todo - usar el enum para los estatus de solicitud
            solicitud.setDesEstatusSolicitud("4");
            solRepository.save(solicitud);
            //pendiente
            ...
            controlRutas.setIdSolcitud(solicitud);

            final Integer idVehiculo = rutas.getIdVehiculo();
            Vehiculos vehiculos = vehiculoRepository
                    .findById(idVehiculo)
                    .orElseThrow(() -> new EntityNotFoundException("No se ha encontrado el vehiculo: " + idVehiculo));
            controlRutas.setVehiculo(vehiculos);

            log.info("ajustando vehiculo a asignado");

            // pendiente agregar el enum del estatus del vehiculo
            vehiculos.setDesEstatusVehiculo("9");
            vehiculoRepository.save(vehiculos);

            log.info(" vehiculo a asignado");
            ruta.setDesServicio(vehiculos.getDesTipoServicio());
            ...
            final Integer idModulo = rutas.getIdModulo();
            ModuloAmbulancia moduloOp = moAmbulanciaRepository.findById(idModulo)
                    .orElseThrow(() -> new EntityNotFoundException("No se ha encontrado el modulo: " + idModulo));

            controlRutas.setModulo(moduloOp);
            ...
            final Integer idTripulacion = rutas.getIdTripulacion();
            Tripulacion tripOp = tripulacionRepository.findById(idTripulacion)
                    .orElseThrow(() -> new EntityNotFoundException("No se ha encontrado la tripulacion: " + idTripulacion));
            controlRutas.setTripulacion(tripOp);
            ...
            Vehiculos vehiculo = vehiculoRepository.getById(controlRutas.getVehiculo().getIdVehiculo());
    ...
    public Respuesta<Integer> editarRuta(Integer idControlRuta, ControlRutasRequest rutaDTO) {
        ...
        try {
            ...
            if (crutaOp.isPresent()) {
                ...
                ruta.setSistema(true);
                List<String> horas = Utility.getHorarioStringByTurno(rutaDTO.getTurno());
                if (horas.size() == 2) {
                    ruta.setTimHorarioInicial(horas.get(0));
                    ruta.setTimHorarioFinal(horas.get(1));
                }
                ...
                contRuta.setMatricula(rutaDTO.getCveMatricula());
