...
public interface DatosAsigRepository extends JpaRepository<DatosAsigEntity, Integer> {
    ...
    @Query(value = "SELECT SCR.ID_CONTROL_RUTA as ID_ASIGNACION, SCR.ID_VEHICULO, " +
            "SCR.ID_RUTA, SCR.ID_SOLICITUD, SV.CVE_ECCO, "
    ...
    @Query("SELECT new mx.gob.imss.mssistrans.ccom.rutas.dto.DatosControlRutaDTO("
            ...
            + " INNER JOIN Vehiculos AS vh ON vh.idVehiculo = cr.vehiculo.idVehiculo "
            + " WHERE cr.sistema = true "
            + " AND cr.activo = true "
