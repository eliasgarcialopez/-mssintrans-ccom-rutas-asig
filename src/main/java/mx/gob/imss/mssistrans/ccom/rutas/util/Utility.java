...
import mx.gob.imss.mssistrans.ccom.rutas.exceptions.UtilityException;
...
public class Utility {
    ...
    private Utility() {
        throw new UnsupportedOperationException("So se puede crear instancias de Utility");
    }
    ...
    public static List<String> getHorarioStringByTurno(Integer idHorario) {
...
//- ID de la ruta (alfanumérico, 15 caracteres, obligatorio)  número consecutivo + siglas de la OOAD(se confirma sea el mismo id generado) el sufijo AMB 
        ...
        StringBuilder consecutivoCeros = new StringBuilder();
        StringBuilder numeroFolio = new StringBuilder();
        ...
        if (longitudConsec < longitud) {
            ...
            for (int i = 1; i <= (longitud - longitudConsec); i++) {
                consecutivoCeros.append("0");
            }

            numeroFolio.append(consecutivoCeros).append(numeroStr);
        } else if (longitudConsec == longitud) {

            numeroFolio.append(consecutivo);

        }
        idGenerado.append(numeroFolio).append(siglas).append(sufijo);
        if (idGenerado.length() > 15)
            throw new UtilityException("Se excedio la longitud del folio, debe ser de 15>Folio" + idGenerado);
