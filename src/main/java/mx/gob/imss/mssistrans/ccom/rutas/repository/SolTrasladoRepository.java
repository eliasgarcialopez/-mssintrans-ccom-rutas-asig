...
public interface SolTrasladoRepository extends JpaRepository<SolTrasladoEntity, Integer> {
    ...
    @Query(value = "SELECT SST.ID_SOLICITUD , SST.NUM_FOLIO_ACEPTACION "
    		...
    		+ " INNER JOIN SINTRANST_MODULO_AMBULANCIA MA ON (SCA.ID_MODULO_AMBULANCIA=MA.ID_MODULO_AMBULANCIA) "
    		+ "	WHERE SST.IND_ACTIVO = 1 AND  SST.IND_SISTEMA = 1  AND MA.IND_ACTIVO  =1 "
    		+ " AND MA.ID_OOAD = ? AND SCA.ID_RUTA = ? "
